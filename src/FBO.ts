...
export default class FBO {
  ...
  protected createTexture(size: number[]): WebGLTexture {
    ...
    this.manager.gl.texParameteri(this.manager.gl.TEXTURE_2D, this.manager.gl.TEXTURE_MAG_FILTER, this.manager.gl.NEAREST);
    this.manager.gl.texParameteri(this.manager.gl.TEXTURE_2D, this.manager.gl.TEXTURE_MIN_FILTER, this.manager.gl.NEAREST);
    this.manager.gl.texParameteri(this.manager.gl.TEXTURE_2D, this.manager.gl.TEXTURE_WRAP_S, this.manager.gl.CLAMP_TO_EDGE);
    this.manager.gl.texParameteri(this.manager.gl.TEXTURE_2D, this.manager.gl.TEXTURE_WRAP_T, this.manager.gl.CLAMP_TO_EDGE);
  ...
  update() {
    this.manager.gl.bindFramebuffer(this.manager.gl.FRAMEBUFFER, this.frameBuffer);
    if (this.feedback) {
    if (this.autoSwap && this.feedback)
      this.swap();
  ...
  onResize(size: number[]){
     ...
     */
      ...
      this.textures[this.write] = this.createTexture(size);
  ...
  output(swap? : boolean): WebGLTexture {
    ...
    if (swap && !this.autoSwap)
      this.swap();

    if (!this.feedback)
      return this.textures[this.write];
    else
